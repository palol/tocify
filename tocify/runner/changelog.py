...
# Match the footer even when one or more blank lines sit between the `---`
# divider and the `*Generated by ...*` line. git-cliff templates and manual
# edits can both reintroduce that blank line, and a strict substring match
# breaks the whole changelog pipeline when it does.
_FOOTER_MARKER_RE = re.compile(
    r"\n---\n\s*\*Generated by \[git-cliff\]\(https://git-cliff\.org\)\.\*"
)
...
def _split_changelog_document(content: str) -> ChangelogDocument:
    ...
    footer_match = None
    for match in _FOOTER_MARKER_RE.finditer(content):
        footer_match = match
    if footer_match is None:
        raise ValueError("Changelog footer marker not found")
    footer_index = footer_match.start()
