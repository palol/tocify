...
- **Changelog**: `tocify changelog` — regenerate `content/changelog.md` from git (requires **git-cliff**, e.g. `brew install git-cliff`). Pipeline: git-cliff → dedupe → add dates → filter; optional Cursor polish when **`CURSOR_API_KEY`** and **`agent`** on PATH are set. Use `--no-cliff` to skip git-cliff, `--no-polish` to skip the agent. Put **`cliff.toml`** at vault root and optionally **`config/changelog_consistency_prompt.txt`** to override the polish prompt.
