"""Tests for editorial weekly brief output (default): reader-facing body, preserved provenance, sanitized why."""
...
class EditorialBriefTests(unittest.TestCase):
    ...
    def test_render_brief_md_editorial_keeps_provenance_in_frontmatter(self) -> None:
        result = {
            "week_of": "2026-02-16",
            "notes": "",
    ...
    def test_merge_brief_frontmatter_editorial_preserves_triage_keys(self) -> None:
        existing = {"date": "2026-02-16", "included": 2, "scored": 10, "triage_backend": "openai", "triage_model": "gpt-4o"}
        merged = merge_brief_frontmatter(
            existing,
            [],
            5,
            12,
            editorial_triage=True,
            triage_backend="gemini",
            triage_model="gemini-2.0-flash",
        )
        self.assertEqual(merged["included"], 5)
        self.assertEqual(merged["scored"], 12)
        self.assertEqual(merged["triage_backend"], "gemini")
        self.assertEqual(merged["triage_model"], "gemini-2.0-flash")
