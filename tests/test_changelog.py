...
class TestSplitChangelogDocument:
    def test_split_strict_footer(self) -> None:
        document = _split_changelog_document(FULL_CHANGELOG)
        assert document.body.startswith("## Website design")
        assert "git-cliff" in document.footer

    def test_split_allows_blank_line_before_footer(self) -> None:
        # git-cliff templates and manual edits sometimes leave a blank line
        # between the `---` divider and the `*Generated by ...*` line. The
        # split must still succeed so the deploy-section pipeline does not
        # fail every run.
        content = FULL_CHANGELOG.replace(
            "\n---\n*Generated by",
            "\n---\n\n*Generated by",
        )
        document = _split_changelog_document(content)
        assert document.body.startswith("## Website design")
        assert "git-cliff" in document.footer

    def test_split_missing_footer_raises(self) -> None:
        import pytest

        with pytest.raises(ValueError, match="footer marker not found"):
            _split_changelog_document("## Section\n- item\n")
